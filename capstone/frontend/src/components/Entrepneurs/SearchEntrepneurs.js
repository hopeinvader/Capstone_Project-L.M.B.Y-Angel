...
import PitchCardList from './PitchCardList'
import Filterbar from '../Filterbar'
import axios from 'axios'
import {loadPitchAPI} from '../../redux/pitch/actions'




export class SearchEntrepneurs extends React.Component {
    constructor(props){
        super(props)
        this.state = {
            value: '',
    ...
    filterSearchBar = (arr, query) => {
        return arr.filter(el=> el.title.toString().toLowerCase().indexOf(query.toLowerCase()) > -1 || el.body.toString().toLowerCase().indexOf(query.toLowerCase()) > -1)
    }
                ...
                <div style={content}>
                    <div className="clearfix mb40">
                        <div style={headerFloat}>
                            <h4 style={textStyle}>Search for Entrepneurs</h4>
                        </div>
                    ...
                    <div className="row">
                        ...
                        <div className="col-lg-8">
                                <PitchCardList pitches={this.state.pitchFilterList}/>
                        </div>
                    </div>
                </div>
            </div>
...
const mapDispatchToProps = (dispatch) => {
    return {
        loadPitch: () => {
            dispatch(loadPitchAPI())
        }
    }
}

export default connect(mapStateToProps, mapDispatchToProps)(SearchEntrepneurs);