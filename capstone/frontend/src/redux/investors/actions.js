...
export function AddAboutActionThunk(about){
    return(dispatch) => {
        axios.post(`${process.env.REACT_APP_API_SERVER}/api/investors/about`, about).then(res => {
            dispatch(formSubmit2())
        })
    }
}

export function formSubmitThunk(username, userInfo, logo, bgImg, facebook_url, linkedin_url){
    return dispatch => {
        return axios.post(`${process.env.REACT_APP_API_SERVER}/api/investors/ucbl`,
            {
                username: username,
                userInfo: userInfo,
                logo: logo,
